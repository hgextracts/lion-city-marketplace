...
export function getKeyHashFromAddress(address: D.Address): string {
  if ("VerificationKey" in address.paymentCredential) {
    return address.paymentCredential.VerificationKey[0];
  }
  throw new Error("Address is not a key-based credential.");
}
